-- Fix RLS policy issues for profile creation
-- This migration adds a SECURITY DEFINER function to handle profile creation
-- which bypasses RLS checks during the initial creation process

-- Function to create profile with role-specific data
...
-- Update RLS policies for municipalities and integrators
-- The current policies check if profile exists with correct role, but during
-- function execution with SECURITY DEFINER, we need to ensure the policies work
-- The function bypasses RLS, but we should also update policies to be more flexible

-- Drop existing INSERT policies for municipalities and integrators
...
-- Create new policies that allow insertion when profile_id matches auth.uid()
-- and the role matches (but don't require the profile to exist first in the same transaction)
...
-- Note: The function with SECURITY DEFINER will bypass RLS entirely,
-- but these updated policies allow direct inserts from the client as well
